...
test('Create POST API Request then create GET API request using dynamic api request file in playwright & typescript', async ({ request }) => {
...
expect(jsonPOSTAPIResponse.booking.bookingdates.checkout)
  .toBe(checkoutDate);
    ...
    const getAPIResponse = await request.get(`/booking/`,{params:{firstname:firstName,lastname:lastName}});
